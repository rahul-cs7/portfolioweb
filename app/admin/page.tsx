...
export default function Admin() {
  ...
  if (!data) return <main className="admin-login"><form className="glass" onSubmit={login}><Brand/><p className="kicker">Secure console</p><h1>Welcome back, Rahul.</h1><p>Enter your private admin token to manage projects, skills, screenshots, links, and messages.</p><input type="password" value={token} onChange={event => setToken(event.target.value)} placeholder="Admin access token" required/><button className="button primary">Enter dashboard <Icon name="arrow"/></button>{error && <p className="admin-error">{error}</p>}<small>Development token: rahulverse-local. Set ADMIN_TOKEN in .env.local before deployment.</small></form></main>;

  return <main className="admin-shell">
    <aside><Brand/><nav><button className={section === "overview" ? "active" : ""} onClick={() => { setSection("overview"); setDraft(null); }}>Overview</button><button className={section === "projects" ? "active" : ""} onClick={() => { setSection("projects"); setDraft(null); }}>Projects</button><button className={section === "skills" ? "active" : ""} onClick={() => { setSection("skills"); setDraft(null); setNotice(""); }}>Skills Galaxy</button><button className={section === "messages" ? "active" : ""} onClick={() => { setSection("messages"); setDraft(null); }}>Messages</button></nav><div className="admin-side-links"><Link href="/#skills">View skills ↗</Link><button onClick={() => { setData(null); setToken(""); }}>Sign out</button></div></aside>
    <section>
